...
- **About/Contact content + interaction pass done**: About has new copy (Canadian spelling, e.g. "colour") and a real portrait (`.portrait-frame` in `src/input.css`, 4:5, 1px border, 3px red box-shadow shift on hover/focus-visible). Contact has a Copy-to-clipboard button next to the mailto link (Clipboard API + `execCommand` fallback, `aria-live="polite"` announcement, 2s auto-reset). `.link-underline` now also triggers on `:focus-visible`, not just `:hover` (site-wide change, improves keyboard-focus parity everywhere the class is used). Self-Portrait card removed from the `/work/` grid.
